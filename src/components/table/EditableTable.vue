...
const filteredList = computed(() => {
    return contacts.value.filter((user: any) => {
...
function deleteItem(item: any) {
    const index = contacts.value.indexOf(item);
    confirm('Are you sure you want to delete this item?') && contacts.value.splice(index, 1);
}
...
function save() {
    if (editedIndex.value > -1) {
        Object.assign(contacts.value[editedIndex.value], editedItem.value);
    } else {
        contacts.value.push(editedItem.value);
    }
    close();
}
